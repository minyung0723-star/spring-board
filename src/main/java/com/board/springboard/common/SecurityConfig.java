...
public class SecurityConfig {
    ...
    public SecurityFilterChain 보안설정(HttpSecurity http) throws Exception{
        http
                ...
                .sessionManagement(sm -> sm
                        ...
                        // 입장 팔찌 기준으로 페스티벌에 접근 가능한 사람인지 판단한다.
                        .sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                .authorizeHttpRequests(auth -> auth.requestMatchers( // 아래 주소는 로그인 토큰 없이 누구나 접근 가능한 경로
                        "/",
                        "/user/login",
                        "/user/register",
                        "/user/check-email",
    ...
    // 클라이언트가 작성한 비밀번호를 스프링에서 만든 비밀번호 암호화 파일을 이용해서 읽을 수 없는 암호화처리
    @Bean
