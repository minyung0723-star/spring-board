 ...
 *  JWT 토큰 발급 검증
 *
 *  Access Token : 30 -> 매 API 요청 해더 or 쿠키 첨부
 *  Refresh Token : 14일 -> 임시저장 / 만료시 재발급용 (=Redis 임시 저장)
...
public class JwtFilter extends OncePerRequestFilter {
     ...
     * 상속받은     전달할것     doFilter = 필터를 실행하겠다
     * 자식만       없다         Internal = OncePerRequestFilter작성되어있는
     * 이용가능
     * @param 요청               브라우저가 보낸 요청 경로
     * @param 응답               브라우저가 요청에 대한 응답을 서버가 전달할 때 들고갈 정보
     * @param 다음필터           우리 필터 작업은 끝났고, 다음 작업으로 바통터치할 때 사용
     * @throws ServletException  서버 연결 에러 발생할 경우
     * @throws IOException       데이터 주고 받을때 발생하는 경우
     */
    @Override
    protected void doFilterInternal(HttpServletRequest 요청, HttpServletResponse 응답, FilterChain 다음필터) throws ServletException, IOException {
        ...
        String token = 쿠키에서토큰꺼내기(요청); // 쿠기에서 access_token 꺼내기 없으면 null 반환
