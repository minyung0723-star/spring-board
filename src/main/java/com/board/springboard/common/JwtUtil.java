...
public class JwtUtil {
    ...
    public void 초기화() {
        ...
        // application.yml 에 작성한 시크릿키값을 암호화 객체로 변환 처리
     ...
     * 로그인 후 엑세스 토큰이 만료되었을 때 14일 유효기간 재발급용
     **/
    ...
    private String 토큰빌드(String email, long 만료시간ms) {
        ...
        return Jwts.builder()                                      // 로그인하면 토큰(=로그인 인증 팔찌) 만들기 시작
                ...
                .signWith(키)                                      // 토큰(=로그인 인증 팔찌) 안에 서명 (위변조 방지)
     ...
     * 이메일만 꺼내오는 기능
     *
     * 멤버쉽 큐알 코드 -> 그 안에 데이터 -> 유저정보를 확인하고 대화하는 것과 같다.
     */
