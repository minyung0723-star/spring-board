...
public class UserService {
    ...
    /**
     * 회원가입을 처리하는 메서드
     * 이메일 중복 확인후 중복이 아닐 경우에만 DB에 회원 정보를 저장
     ...
     * 처리 순서 :
     * 1. 업로드된 파일이 비어있는지 확인
     * 2. 저장 폴더가 없으면 자동 생성(서버 재시작 재부팅 시 안정하게 이미지를 가져올 수 있다.
     * 3. 파일명 충돌 방지를 위해 UUID + 원본 확장자로 파일명 생성
     * 4. 서버 디스크에 파일 저장
     * 5. DB의 profile_img 컬럼에 웹 접근 경로 저장
     * 6. 세션 갱신을 위해 수정된 최신 User 데이터 객채 반환
     * @param loginUser 현재 로그인 되어있는 세션에서 꺼낸 현재 로그인 유저의 정보가 담겨있는 변수공간의 명칭
     * @param imageFile <input type="file">로 전달된 MultipartFile
     * @param uploadPath 파일을 저장할 서버 절대경로(application.properties 에서 설정하고, 설정된 키이름 경로 가져올것)
    ...
    public User 프로필사진업로드(User loginUser, MultipartFile imageFile, String uploadPath) throws IOException {
        ...
        // 5. DB 애 저장할 웹 접근 경로 설정
        String 웹경로 = "/uploads/profile/" + 저장할파일;

        // 6. DB 업데이트 : 해당 유저의 profile.img 컬럼에 웹 경로 저장
